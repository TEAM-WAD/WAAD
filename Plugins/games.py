...
''')
   ...
   if text == "سورس" or text == "السورس":
    ...
    try:
        channel_photo_url = "https://t.me/EgyCodes/photo"
        return m.reply_photo(
   ...
   if text.lower() in ["بوت", "حمزه"]:
    try:
        photo_enabled = r.get(f'photo_status:{m.chat.id}') != 'disabled' if r.exists(f'photo_status:{m.chat.id}') else True
        
        reply_markup = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("Abu Hamza", url="t.me/FFJFF5")
            ],
            [
                InlineKeyboardButton("Channel Source", url=f"https://t.me/EgyCodes")
            ]
        ])
